...
def speed_func(geojson_row, label_type="sn5", verbose=True):
    ...
    speed_final_mph = speed_init_mph * road_surface_dict[surface] * bridge_dict[bridge]
...
###############################################################################
def speed_to_burn_val(speed, min_speed=15, max_speed=65.0, min_road_burn_val=127, mask_max=255):
    """Convert speed estimate to mask burn value between 0 and 255"""

    bw = mask_max - min_road_burn_val
    burn_val = min_road_burn_val + bw * ((speed - min_speed) / (max_speed - min_speed))
    return max(burn_val, min_road_burn_val)


###############################################################################
def speed_to_bins_bg(speed_mph, bin_size_mph=10.0, channel_value_mult=1.0):
...
def add_speed_to_geojson(
    ...
    with open(geojson_path_in, "r+") as f:
        ...
        for i, geojson_row in enumerate(geojson_data["features"]):
            ...
            speed_mph, speed_mps = speed_func(geojson_row, label_type=label_type, verbose=verbose)
            if verbose:
    ...
    if len(bad_row_idxs) > 0:
        ...
        geojson_data["features"] = [e + "\n" for i, e in enumerate(geojson_data["features"]) if i not in bad_row_idxs]
...
###############################################################################
def update_geojson_dir_speed(
    geojson_dir_in, geojson_dir_out, label_type="sn5", suffix="_speed", nmax=1000000, verbose=True, super_verbose=False
):
    ...
    for i, json_file in enumerate(json_files):
        ...
        geojson_path_out = os.path.join(geojson_dir_out, root + suffix + "." + ext)
        sarr, sset = add_speed_to_geojson(geojson_path_in, geojson_path_out, label_type=label_type, verbose=verbose)
...
def create_speed_gdf(
    ...
    projGDF = osmnx.project_gdf(inGDF)
    if verbose:
...
##############################################################################
def gdf_to_array(
    gdf, im_file, output_raster, burnValue=150, mask_burn_val_key="", compress=True, NoData_value=0, verbose=False
):
    """
    ...
    if compress:
        target_ds = gdal.GetDriverByName("GTiff").Create(
            output_raster, gdata.RasterXSize, gdata.RasterYSize, 1, gdal.GDT_Byte, ["COMPRESS=LZW"]
        )
    else:
        target_ds = gdal.GetDriverByName("GTiff").Create(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--geojson_dir_in", default="", type=str, help="Input directory of geojsons")
    parser.add_argument("--geojson_dir_out", default="", type=str, help="Output directory of updated geojsons")
    parser.add_argument("--label_type", default="sn5", type=str, help="sn3', 'sn5', or 'osm'")
    parser.add_argument("--suffix", default="_speed", type=str, help="suffix for output")
    args = parser.parse_args()

    update_geojson_dir_speed(
        args.geojson_dir_in, args.geojson_dir_out, label_type=args.label_type, suffix=args.suffix, verbose=True
    )
