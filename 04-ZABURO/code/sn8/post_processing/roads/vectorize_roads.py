...
from sn8.post_processing.roads.utils import sknw, sknw_int64
...
def parse_args():
    ...
    parser.add_argument("--image_filename", help="the raw image filename", required=False, type=str)
    parser.add_argument(
        "--im_dir",
    ...
    parser.add_argument("--out_shp", help="output filename for the shapefile", required=False)
    parser.add_argument(
...
def G_to_wkt(
    ...
    if add_small and len(terminal_points) > 1:
        small_segs, good_pairs, good_coords = add_small_segments(G, terminal_points, terminal_lines, verbose=verbose)
...
def build_wkt_linestrings(
    ...
    wkt_list = G_to_wkt(G, add_small=add_small, verbose=verbose, super_verbose=super_verbose)
