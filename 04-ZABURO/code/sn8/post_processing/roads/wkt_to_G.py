...
from sn8.post_processing.roads.utils import make_logger, rdp
...
def parse_args():
    ...
    parser.add_argument("--min_subgraph_length_pix", type=int, default=20)
    parser.add_argument("--min_spur_length_m", type=int, default=10)
    parser.parse_args()
    ...
    for i, lstring in enumerate(wkt_list):
        ...
        for j, (x, y) in enumerate(zip(xs, ys)):
            ...
            # if not first node in edge, retrieve previous node and build edge
                ...
                edge_length = abs(proj - proj_prev)  # Euclidean distance between prev_loc and loc
                # make linestring
...
###############################################################################
def get_edge_geo_coords_single_threaded(G, im_file, remove_pix_geom=True, fix_utm_zone=True, verbose=False):
...
def wkt_to_G(params):
    ...
    # この時点では edge の geometry には意味があるデータが入っていないので捨てる
    # 放っておいても捨てられるけど明示的に行う
    for u, v, attr_dict in G1.edges(data=True):
        # assert np.allclose(
        #     np.asarray(attr_dict["geometry"].xy),
        #     [
        #         (G1.nodes[u]["x"], G1.nodes[v]["x"]),
        #         (G1.nodes[u]["y"], G1.nodes[v]["y"]),
        #     ],
        # )
        del attr_dict["geometry"]

    # G_projected has UTM CRS
    ...
    if simplify_graph:
        ...
        # すでに UTM になっているので projection はいらない
        # # Gout = G0
...
if __name__ == "__main__":
    ...
    console, logger1 = make_logger.make_logger(log_file, logger_name="log", write_to_console=bool(log_to_console))
