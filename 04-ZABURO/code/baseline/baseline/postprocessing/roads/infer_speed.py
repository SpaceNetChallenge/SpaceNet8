...
from utils import apls_plots, make_logger
...
def get_linestring_midpoints(geom):
    ...
    for i in range(N - 1):
        ...
        dls.append(dl)
    return np.array(x_mids).astype(int), np.array(y_mids).astype(int), np.array(dls)


###############################################################################
def get_patch_speed_singlechannel(
    patch, conv_dict, percentile=80, verbose=False, super_verbose=False
):
    """
...
def get_edge_time_properties(
    ...
    for j, (x, y, dl_pix) in enumerate(zip(x_mids, y_mids, dls)):
        ...
        if multichannel:
            ...
            patch = mask[y0:y1, x0:x1]
            z = 0
            speed_mph_seg, _ = get_patch_speed_singlechannel(
                patch,
                conv_dict,
                percentile=percentile,
                verbose=verbose,
...
def infer_travel_time(params):
    ...
    if save_shapefiles:
        ...
        ox.save_graph_shapefile(
            G_out, filename=im_root, folder=graph_dir_out, encoding="utf-8"
        )
...
def parse_args():
    ...
    parser.add_argument("--graph_speed_dir", type=str, required=True)
...
if __name__ == "__main__":
    ...
    graph_dir = (
        args.graph_dir
    )  # this is read only. they are the graphs from the last step. graphs_nospeed
    ...
    console, logger1 = make_logger.make_logger(
        log_file, logger_name="log", write_to_console=bool(log_to_console)
    )
    ...
    if num_classes > 1:
        ...
        conv_df, conv_dict = load_speed_conversion_dict_contin(speed_conversion_file)
    ...
    if N_plots > 0:
        ...
        plot_width_key, plot_width_mult = (
            "inferred_speed_mph",
            0.085,
        )  # 0.08  # variable width
        ...
        for i, image_path in enumerate(image_names):
            ...
            pkl_path = os.path.join(
                graph_speed_dir,
                image_name.replace("_preimg.tif", "_roadspeedpred.gpickle"),
            )
