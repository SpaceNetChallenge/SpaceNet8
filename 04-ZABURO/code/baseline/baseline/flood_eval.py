...
def make_prediction_png(image, postimage, gt, prediction, save_figure_filename):
    ...
    combined_mask_cmap = colors.ListedColormap(
...
if __name__ == "__main__":
    ...
    val_dataset = SN8Dataset(
        in_csv, data_to_load=["preimg", "postimg", "flood"], img_size=img_size
    )
        ...
        for i, data in enumerate(val_dataloader):
            ...
            if save_preds_dir is not None:
                ...
                output_tif = os.path.join(
                    save_preds_dir,
                    os.path.basename(
                        current_image_filename.replace(".tif", "_floodpred.tif")
                    ),
                )
            ...
            for j in range(4):  # there are 4 classes
                gt = np.where(
                    gt_flood == (j + 1), 1, 0
                )  # +1 because classes start at 1. background is 0
            ...
            if save_fig_dir != None:
                save_figure_filename = os.path.join(
                    save_fig_dir,
                    os.path.basename(current_image_filename)[:-4] + "_pred.png",
                )
                make_prediction_png(
                    preimg, postimg, gt_flood, flood_prediction, save_figure_filename
                )
