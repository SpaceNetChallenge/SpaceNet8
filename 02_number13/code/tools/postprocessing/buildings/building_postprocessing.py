...
def main_w_flood(im_dir,
    ...
    """
    quads = get_quads()
    bld_preds = glob.glob(root_dir + "/building_*.png")
    print(f"postprocessing {len(bld_preds)} tiles")

    cols = ['ImageId','Object','Wkt_Pix', 'Flooded']
    record_list = []
    count = 0

    for in_file in bld_preds:
        name_root = os.path.basename(in_file).replace("building_", '')
        #print(in_file)
        # morphological opening
        im_file = os.path.join(im_dir, os.path.basename(in_file).replace('.png', '.tif').replace('building_', ''))
        ...
        raster_srs = osr.SpatialReference()
        raster_srs.ImportFromWkt(ds.GetProjectionRef())
        in_arr = io.imread(in_file)
        ds = None
        #print("morph opening...")
        building_arr = opening(in_arr, square(square_size))

        flood_arr = io.imread(os.path.join(flood_dir, os.path.basename(in_file).replace("building", "flood")))
        if flood_arr.shape[-1]==3:
            flood_arr = flood_arr[:, :, 0]
        flood_arr[flood_arr==255]=2
        #plt.subplot(121)
        #plt.imshow(building_arr)
        #plt.subplot(122)
        #plt.imshow(flood_arr)
        #plt.show()
        ...
        if len(feats) == 0: # no buildings detecting in the tile, write no prediction to submission
            record_list.append([name_root.replace('.png', ''), 'Building', 'POLYGON EMPTY',  'Null'])
        else:
            flood_val = [True if f['properties']['mask_val'] == 2 else False for f in feats]
            flood_val_perc_whole = (sum(flood_val)/len(flood_val)*100)

            if flood_val_perc_whole>50:
               flood_hard = [True]*len(flood_val)
            else:
               flood_hard = [False]*len(flood_val)
            '''
            feat_pix = [geo_coords_to_image_coords(image_geotran, f['geometry']) for f in feats]
            feat_in_quads={i:[] for i in range(len(quads))}
            for j, fpix in enumerate(feat_pix):
                for k, v in feat_in_quads.items():
                    if shapely.wkt.loads(fpix).intersects(quads[k]):
            ...
            for k, v in feat_in_quads.items():
                ...
                for building in  v:
                    ...
                    #    record_list.append([name_root.replace('.png', ''), 'Building',  building['geom'], True])
                    #else:
                    if flood_val_perc>30:
                        record_list.append([name_root.replace('.png', ''), 'Building',  building['geom'], True])

                    else:
                        record_list.append([name_root.replace('.png', ''), 'Building',  building['geom'], False])

            '''
            flood_val =[i1 or i2 for i1, i2 in zip(flood_val, flood_hard)]
            for flood_val_c, f in zip(flood_hard, feats):
                wkt_image_coords = geo_coords_to_image_coords(image_geotran, f['geometry'])
                record_list.append([name_root.replace('.png', ''), 'Building',  wkt_image_coords, flood_val_c])
        count+=1
...
#conda run -n gdal python building_postprocessing.py --im_dir ~/data/Germany_Training_Public/PRE-event/ --foundation_pred_dir ~/data/tiny_masks/ --flood_pred_dir ~/data/tiny_masks/ --out_submission_csv subs_tiny.csv
