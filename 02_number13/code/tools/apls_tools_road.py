...
def create_buffer_geopandas(geoJsonFileName, bufferDistanceMeters=2, 
    ...
    inGDF['speed'] = inGDF['inferred_speed_mph'].values.round().astype('float') 
    if len(inGDF) == 0:
        return []
...
def gdf_to_array(gdf, im_file, output_raster, burnValue=150):
    ...
    for i,geomShape in enumerate(gdf['geometry'].values):
        speed  = gdf.iloc[i].speed
        outFeature = ogr.Feature(featureDefn)
        outFeature.SetGeometry(ogr.CreateGeometryFromWkt(geomShape.wkt))
        outFeature.SetField(burnField, speed)
        outLayer.CreateFeature(outFeature)
        outFeature = 0
    gdal.RasterizeLayer(target_ds, [1], outLayer, options = ["ATTRIBUTE=burn"])
    outLayer = 0
    outDatSource = 0
    tmp = 0
