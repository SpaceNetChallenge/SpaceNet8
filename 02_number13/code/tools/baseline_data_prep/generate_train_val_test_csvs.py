...
def gather_images_masks(image_dir):
    ...
    mask_types = ["flood", "building", "road"]
    images = []
    masks = []
...
def make_train_val_csvs(image_dirs,
    ...
    for d in image_dirs:
        ...
        bldgs = glob.glob(os.path.join(d, "annotations", "masks_png", "building*.png"))
        roads = glob.glob(os.path.join(d, "annotations", "masks_png", "road*.png"))
        flood = glob.glob(os.path.join(d, "annotations", "masks_png", "flood*.png"))
        roadspeed = glob.glob(os.path.join(d, "annotations", "masks_png", "road*.png"))
        pre = glob.glob(os.path.join(d, "PRE-event", "*.tif"))
        post = glob.glob(os.path.join(d, "POST-event-warped", "*.tif"))
    ...
    kf = KFold(n_splits=7)
    for i, (train_idxs, val_idxs) in enumerate(kf.split(idxs)):
        write_csv(all_images, all_masks, train_idxs, f"{out_csv_basename}_train_fold{i+1}.csv")
        write_csv(all_images, all_masks, val_idxs, f"{out_csv_basename}_val_fold{i+1}.csv")
...
def match_im_label(anno, bldgs, roads, floods, roadspeeds, pre, post):
    ...
    for i in anno:
        ...
        build = [j for j in bldgs if "building_" in j and f"_{tileid}.png" in j][0]
        road = [j for j in roads if "road_" in j and f"_{tileid}.png" in j][0]
        flood = [j for j in floods if "flood_" in j and f"_{tileid}.png" in j][0]
        speed = [j for j in roads if "road_" in j and f"_{tileid}.png" in j][0]
