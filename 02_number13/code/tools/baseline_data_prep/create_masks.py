...
def write_png(arr, output_png):
    io.imsave(output_png, arr, check_contrast=False)
...
def create_mask_for_tile(pre_image,
    ...
    mask_types = ["binary_road", "binary_building", "flood"]
    ...
    if "binary_building" in mask_types:
        out_imagename = f"building_{os.path.basename(pre_image).split('.')[0]}.png"
        output_tif = os.path.join(output_image_directory, out_imagename)
        write_png((building_binary_arr*255).astype('uint8'), output_tif)
    ...
    if "binary_road" in mask_types:
        # road mask tile
        out_imagename = f"road_{os.path.basename(pre_image).split('.')[0]}.png"
        output_tif = os.path.join(output_image_directory, out_imagename)
        write_png((road_binary_arr*255).astype('uint8'), output_tif)
        #write_geotiff(output_tif, preim_ncols, preim_nrows,
        #              xmin, xres, ymax, yres,
    ...
    if "flood" in mask_types:
        ...
        flood_arr = np.zeros((3, preim_ncols, preim_nrows))
        flood_arr[0][building_flood_arr==2] = 1
        flood_arr[1][road_flood_arr==2] = 1
        flood_arr = (np.transpose(flood_arr, (1, 2, 0))*255).astype('uint8')
        # flood mask tile
        out_imagename = f"flood_{os.path.basename(pre_image).split('.')[0]}.png"
        output_tif = os.path.join(output_image_directory, out_imagename)
        write_png(flood_arr, output_tif)
        #write_geotiff(output_tif, preim_ncols, preim_nrows,
        #              xmin, xres, ymax, yres,
...
if __name__ == "__main__":
    ...
    for i in range(len(pre_images)):
        ...
        output_image_directory = os.path.join(out_dir_root, "masks_png")
