...
from utils.plots import write_gt_pred_images, write_pred_images, write_best_pred_tiffs, write_pred_tiff
...
def run(weights_dir="./",
    ...
    if task == 'test':
        ...
        dataloader, dataset = create_dataloader(dataset_path, data_to_load, task='test', batch_size=batch_size // WORLD_SIZE,
                                                    hyp=hyp, augment=False, cache=False, rank=LOCAL_RANK, workers=workers)
    ...
    best_save_dir = "./runs/best"
    w2 = Path(best_save_dir)
    w2.mkdir(parents=True, exist_ok=True)

    for item in ["building", "road", "flood"]:
        if is_main_process():
            LOGGER.info(f"Evaluating [{item}]...")
        weights = os.path.join(weights_dir, "best_" + item + ".pt")
        ...
        for i, data in enumerate(dataloader):
            ...
            flood_preds0 = F.softmax(flood_preds, dim=1)
            flood_preds = flood_preds0.argmax(dim=1)
            flood_preds = torch.where(((flood_preds0.argmax(dim=1) == 2) & (flood_preds0[:,2,:,:] < flood_thresh)), 1, flood_preds)
            flood_preds = torch.where(((flood_preds0.argmax(dim=1) == 4) & (flood_preds0[:,4,:,:] < flood_thresh)), 3, flood_preds)
            ...
            if tiffs:
                if item == "building":
                    write_best_pred_tiffs(preimg_filenames, item, building_preds, roadspeed_sn_preds, flood_preds, save_dir)
                elif item == "road":
                    write_best_pred_tiffs(preimg_filenames, item, building_preds, roadspeed_sn_preds, flood_preds, save_dir)
                elif item == "flood":
                    write_best_pred_tiffs(preimg_filenames, item, building_preds, roadspeed_sn_preds, flood_preds, save_dir)        
    ...
    for i, data in enumerate(dataloader):
        ...
        for k in range(size):
            building_pred_path = os.path.join(save_dir, os.path.basename(preimg_filenames[k]).replace('.tif', '_buildingpred.tif'))     
            building_pred = open_tiff(building_pred_path).astype(np.float32)/255.0
            roadspeed_pred_path = building_pred_path.replace('_buildingpred.tif', '_roadspeedpred.tif')
            roadspeed_pred = open_tiff(roadspeed_pred_path).astype(np.float32)/255.0
            flood_pred = open_tiff(building_pred_path.replace('_buildingpred.tif', '_floodpred.tif'))
            flood_building_pred = np.where((flood_pred == 1) | (flood_pred == 2), 1, 0)
            
            flood_road_pred = np.where((flood_pred == 3) | (flood_pred == 4), 1, 0)
            # OR operations
            roadspeed_pred[7] = np.bitwise_or(roadspeed_pred[7].astype(np.int32),flood_road_pred).astype(np.float32)
            building_pred = np.bitwise_or(building_pred.astype(np.int32), flood_building_pred).astype(np.float32) 
            new_flood_building_pred = building_pred
            new_flood_road_pred = roadspeed_pred[7]
            # create flood map
            flood_map = np.where((flood_pred == 2) | (flood_pred == 4), 1, 0)
        
            # assign class
            new_flood_pred = assign_flood_class(preimgs[k], flood_map, new_flood_building_pred, new_flood_road_pred)
            write_pred_tiff(preimg_filenames[k], building_pred, roadspeed_pred, new_flood_pred, best_save_dir)

    return 0


def assign_flood_class(img, flood_map, building_pred, road_pred):
    new_flood_pred = np.zeros(img.shape[1:3])  # (H, W)
    pos = (building_pred == 1)
...
def parse_opt():
    ...
    parser.add_argument('--weights-dir', type=str, default=ROOT, help='weights directory')
    parser.add_argument('--batch-size', type=int, default=4, help='batch size')
    parser.add_argument('--dataset-path', type=str, default=ROOT / 'data/SN8_flood/test', help='dataset path')
    parser.add_argument('--task', default='val', help='train, val, test, speed or study')
    parser.add_argument('--exist-ok', action='store_true', help='existing project/name ok, do not increment')
    parser.add_argument('--device', default='', help='cuda device, i.e. 0 or 0,1,2,3 or cpu')
    parser.add_argument('--flood-thresh', type=float, default=0.9, help='threshold for flood decision')
