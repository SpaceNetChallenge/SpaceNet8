...
def main_w_flood(root_dir,
    ...
    if out_shapefile_dir is not None:
        isExist = os.path.exists(out_shapefile_dir)
        if not isExist:
            os.makedirs(out_shapefile_dir)
        print("The out_shapefile_dir directory is created!")
    ...
    df_sol = df[['ImageId','Object','Wkt_Pix','Flooded']]
    df_sol[['length_m','travel_time_s']] = 'Null'
    df_sol = df_sol.drop_duplicates(keep=False)
    df_sol.to_csv(out_submission_csv, index=False)
