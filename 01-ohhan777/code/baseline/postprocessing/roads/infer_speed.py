...
def infer_travel_time(params):
    ...
    if save_shapefiles:
        ...
        # ox.save_graph_shapefile(G_out, filename=im_root, folder=graph_dir_out, # modified by HHLEE
        #                         encoding='utf-8')
        for node, data in G_out.nodes(data=True):
            if 'osmid' in data:
                data['osmid_original'] = data.pop('osmid')
        ox.save_graph_shapefile(G_out, filepath=graph_dir_out, # modified by HHLEE
                                encoding='utf-8')
    return G_
...
def add_travel_time_dir(graph_dir, mask_dir, conv_dict, graph_dir_out,
    ...
    # image_names = sorted([z for z in os.listdir(mask_dir) if z.endswith('.tif')]) commented out by HHLEE
    image_names = sorted([z for z in os.listdir(mask_dir) if z.endswith('roadspeedpred.tif')])
...
if __name__ == "__main__":
    ...
    if os.path.exists(os.path.join(args.eval_csv, 'MysteryCity_Test_Private/MysteryCity_Test_Private_label_image_mapping.csv')):
        list_path = os.path.join(args.eval_csv, 'MysteryCity_Test_Private/MysteryCity_Test_Private_label_image_mapping.csv')
    else:
        list_path = os.path.join(args.eval_csv, 'Louisiana-West_Test_Public/Louisiana-West_Test_Public_label_image_mapping.csv')
    
    eval_csv = list_path # only used for plotting...
    ...
    num_channels = 3
    log_to_console = 1
    speed_conversion_file = "baseline/postprocessing/roads/speed_conversion_key.csv"
    num_classes = 8
    mask_prefix = ""
    ...
    verbose = False
    n_threads = 1 # insted of 2 by HHLEE
    ##########    
    ...
    if N_plots > 0:
        ...
        with open(eval_csv, newline='') as csvfile:
            ...
            for row in reader:
                ...
                preimg_path = os.path.join(os.path.dirname(eval_csv),'PRE-event/' +  row["pre-event image"])
                rgb_image_filepaths.append(preimg_path)
        ...
        for i,image_path in enumerate(image_names):
            ...
            pkl_path = os.path.join(graph_speed_dir, image_name.replace("_preimg.tif",  ".gpickle"))
