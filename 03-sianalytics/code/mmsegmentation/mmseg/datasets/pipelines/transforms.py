...
def my_impad(img,
    ...
    padding = (padding[2] // 2, padding[3] // 2, padding[2] // 2,
               padding[3] // 2)
...
class PhotoMetricDistortion(object):
    ...
    def brightness(self, img):
        ...
        if random.randint(2):
            for idx in range(0, img.shape[-1], 3):
                pre_img = img[:, :, idx:idx + 3]
                pre_img = self.convert(
                    pre_img,
                    beta=random.uniform(-self.brightness_delta,
                                        self.brightness_delta))
                img[:, :, idx:idx + 3] = pre_img
            return img
        return img
    ...
    def saturation(self, img):
        ...
        if not random.randint(2):
            return img

        alpha = random.uniform(self.saturation_lower, self.saturation_upper)
        for idx in range(0, img.shape[-1], 3):
            hsv = mmcv.bgr2hsv(img[:, :, idx:idx + 3])
            hsv[:, :, 1] = self.convert(hsv[:, :, 1], alpha=alpha)
            img[:, :, idx:idx + 3] = mmcv.hsv2bgr(hsv)
        return img
...
class RoadDialation(object):
    ...
    def __init__(self, prob=0.5, kernel_size=3, iteration=2, classes=[3, 4]):
        self.prob = prob
    ...
    def __call__(self, results):
        ...
        dilate = True if np.random.rand() < self.prob else False
        if dilate:
            # flip segs
            for key in results.get('seg_fields', []):
                # use copy() to make numpy stride positive
                ...
                for cls in self.classes:
                    road = (gt_seg == cls).astype('uint8')[..., np.newaxis]
                    road = cv2.dilate(
                        road, self.kernel, iterations=self.iteration)
                    gt_seg[road == 1] = road[road == 1] * cls
...
class MergeClasses(object):
    ...
    def __init__(self, classes=[[1, 2], [3, 4]]):
        self.classes = classes
    ...
    def __call__(self, results):
        ...
        for key in results.get('seg_fields', []):
            ...
            gt_seg = results[key]
            new_canvas = np.zeros_like(gt_seg)
            canvases = []
            for cls in self.classes:
                cls_map = []
                for c in cls:
                    m = (gt_seg == c)
                    cls_map.append(m)
                canvases.append(np.logical_or(*cls_map))
            for i, cl in enumerate(canvases):
                new_canvas[cl == 1] = cl[cl == 1] * (i + 1)

            results[key] = new_canvas
        return results

    def __repr__(self):
        return self.__class__.__name__ + f'(classes={self.classes})'
...
class RandomFog(object):
    ...
    def __init__(self,
                 ...
                 alpha_coef=0.1):
        self.prob = prob
        self.coef_lower = coef_lower
        self.coef_upper = coef_upper
        self.alpha_coef = alpha_coef
        self.color = (255, 255, 255)
    
    def add_fog(self, img):
        fog_coef = random.uniform(self.coef_lower, self.coef_upper)
        ...
        for haze_points in haze_list:
            ...
            overlay = img.copy()
            output = img.copy()
            alpha = self.alpha_coef * fog_coef
            rad = hw // 2
            point = (x + hw // 2, y + hw // 2)
            cv2.circle(overlay, point, int(rad), self.color, -1)
    ...
    def __call__(self, results):
        ...
        img = results['img']
        fog = True if np.random.rand() < self.prob else False
        if fog:
            # random  fog
            img = self.apply(img)
