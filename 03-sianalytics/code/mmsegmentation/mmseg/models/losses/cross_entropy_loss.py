...
def _calculate_adaptive_class_weight(label, num_classes):
    class_weight = []
    for i in range(num_classes):
        class_weight.append(torch.sum(label == i))
    class_weight = label.new_tensor(class_weight)
    class_weight = 1 - class_weight / label.numel()
    class_weight = class_weight / sum(class_weight)
    return class_weight


@LOSSES.register_module()
class AdaptiveCrossEntropyLoss(CrossEntropyLoss):

    def __init__(self, loss_name='loss_adaptive_ce', **kwargs):
        super(AdaptiveCrossEntropyLoss, self).__init__(
            class_weight=None, loss_name=loss_name, **kwargs)
    ...
    def forward(self,
        ...
        class_weight = _calculate_adaptive_class_weight(label, num_classes)
        # Note: for BCE loss, label < 0 is invalid.
        loss_cls = self.loss_weight * self.cls_criterion(
            cls_score,
            label,
