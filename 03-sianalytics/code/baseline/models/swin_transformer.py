...
def build_swinl():
    model = SwinTransformer(
        ...
        init_cfg=dict(checkpoint='models/swin_large_patch4_window7_224_22k_20220412-aeecf2aa.pth'))
    model.init_weights()
...
def build_swinb():
    model = SwinTransformer(
        ...
        act_cfg=dict(type='GELU'),
        norm_cfg=dict(type='LN', requires_grad=True),
        init_cfg=dict(checkpoint='checkpoints/simmim_swin_base_img192_window6.pth'))
    model.init_weights()
...
def build_swinb_22k():
    model = SwinTransformer(
        ...
        act_cfg=dict(type='GELU'),
        norm_cfg=dict(type='LN', requires_grad=True),
        init_cfg=dict(checkpoint='checkpoints/swin_base_patch4_window7_224_22k_20220317-4f79f7c0.pth'))
    model.init_weights()
...
from models.pytorch_zoo.abstract_model import AbstractModel, ConvBottleneck, UnetDecoderBlock
class SwinLU(AbstractModel):
    def __init__(self, num_classes, num_channels=3):
        super().__init__()
        # self.filters = [192, 384, 768, 1536]
        self.filters = [192, 384, 768, 1536, 1536]
...
class SwinBU(SwinLU):
    def __init__(self, num_classes, num_channels=3):
        ...
        # self.filters = [192, 384, 768, 1536]
        self.filters = [128, 256, 512, 1024, 1024]
        self.num_channels = num_channels
        self.num_classes = num_classes
        assert(isinstance(self.num_classes, int) or isinstance(self.num_classes, list))
        ...
        self.backbone = build_swinb()
        
class SwinBU22K(SwinLU):
    def __init__(self, num_classes, num_channels=3):
        AbstractModel.__init__(self)
        # self.filters = [192, 384, 768, 1536]
        self.filters = [128, 256, 512, 1024, 1024]
        ...
        self.last_upsample = UnetDecoderBlock(self.filters[0], self.filters[0] // 2, 4)
