...
def parse_args():
    ...
    parser.add_argument('--band',
                        help='default channel for skeleton',
                        default=7, type=int)
    parser.add_argument('--band-val',
                        help='use value than channel index',
                        action='store_true')
    parser.add_argument('--suffix',
                        help='default suffix for filename',
                        default='roadspeedpred')
...
def make_skeleton(img_loc, thresh, debug, fix_borders, replicate=5,
    ...
    if use_band_val:
        img = ds.ReadAsArray()
        img = (img==skeleton_band).astype('uint8')
    else:
...
if __name__ == "__main__":
    ...
    if not os.path.exists(im_dir):
        ...
        print(im_dir)
        print(args.suffix)
        images = glob.glob(im_dir + f"/*{args.suffix}.tif")
    ...
    for img_loc in images:
        ...
        img, ske = make_skeleton(img_loc, thresh=0.3, debug=False, 
                                    ...
                                    max_out_size=(200000, 200000),
                                    num_classes=1,
                                    skeleton_band=args.band,
                                    kernel_blur=27,
                                    min_background_frac=0.2,
                                    use_band_val=args.band_val,
                                    verbose=True)
