...
def _clean_sub_graphs(G_, min_length=80, max_nodes_to_skip=100,
    ...
    sub_graphs =list(G_.subgraph(c) for c in nx.connected_components(G_))
    bad_nodes = []
    if verbose:
...
def make_graphs(G_gt_, G_p_,
    ...
    for i, (u, v, key, data) in enumerate(G_p_.edges(keys=True, data=True)):
        ...
        try:
            # edited part
            # line = data['geometry']
            if 'geometry_utm_wkt' in data:
                data['geometry'] = data['geometry_utm_wkt']
            line = data['geometry']
        except:
            # edited part
            # line = data[0]['geometry']

            if 'geometry_utm_wkt' in data[0]:
                data[0]['geometry'] = data[0]['geometry_utm_wkt']
            line = data[0]['geometry']
        if type(line) == str:  # or type(line) == unicode:
            data['geometry'] = shapely.wkt.loads(line)
...
#            #########################
    ...
    if test_method == 'gt_json_prop_pkl':
        ...
        for i, f in enumerate(name_list):
            ...
            if f.startswith('spacenetroads'):
                ...
                outroot = f.split('.')[0]
                im_file = os.path.join(im_dir,'*'+ outroot+ '.tif')
                im_file = glob(im_file)[0]

                prop_file = os.path.join(
                        prop_dir, '*'+ outroot + '.gpickle')
                prop_files = glob(prop_file)
                if prop_files:
    ...
    if test_method == 'gt_json_prop_json':
        ...
        for f in name_list:
            ...
            # skip empty ground truth graphs
            if G_gt_init is None or len(G_gt_init.nodes()) == 0:
                continue
...
#        ##################
    ...
    print(("Tot APLS = np.mean(APLS_arr:", np.mean(df['APLS'].values[:-2])))
    return
